...
### v1.3 (2026-03-10)
- 新增联网搜索开关，普通对话与 Agent 模式均支持，按需开启节省额度
- Agent 角色分配优化：reasoner 最多1次，aggregator 只能1个，新增 searcher 角色
- 统一默认模型为 DeepSeek-V3.2，降低推理成本
- 增加 max_token 至 20000，修复长内容生成中途截断问题
- 新增代理配置入口，首次运行可配置 https_proxy
- 修复ai思考时用户输入的命令不出现的问题
...
### v1.1 (2026-03-09)
- 修复前端显示代码块时渲染重叠的问题
- 输出完成后改用 st.markdown() 渲染，修复代码块/表格等 Markdown 格式无法高亮显示的问题
- 修复内容直接注入 HTML 的 XSS 缺陷
- 修复 Agent 模式因状态机顺序错误导致汇总阶段永久死锁的 P0 问题
- 停止按钮补全真正的线程取消机制
- 停止后保留已完成子任务内容到历史
- 修复侧边栏文件句柄泄漏
